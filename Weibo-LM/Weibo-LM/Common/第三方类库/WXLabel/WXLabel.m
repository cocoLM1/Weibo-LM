...
-(NSString *)description{
    return [NSString stringWithFormat:@"%@ : %@",_user.name,__text];
}
...
-(BOOL)modelCustomTransformFromDictionary:(NSDictionary  *)dic{
    ...
    for (NSString *string in array) {
        NSArray *emotions = [NSArray array];
        
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"chs = %@",string];
        NSArray *result = [emotions filteredArrayUsingPredicate:predicate];
        ...
        if (result.count == 1) {
            ...
            weiboText = [weiboText stringByReplacingOccurrencesOfRegex:string withString:imageURL];
        }
